...
export const cleanText = (text: string) => text.trim().replace(cleanRegex, "-$&").toLowerCase();
...
export const getMetaAttribute = (document: Document, property: string, attribute: string) => document.querySelector(`meta[property=${property}]`)?.getAttribute(attribute) || undefined;

export const isObject = (value: any): value is CssObject => value != null && typeof value === "object" && !Array.isArray(value);
